...
async function check() {
  const { data, error } = await supabase.from('employees').select('id, employee_code, full_name, email, designation, status');
  if (error) {
    console.error("Error:", error);
    return;
  }
  console.log("Employees data count:", data ? data.length : 0);
